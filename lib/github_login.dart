...
final _autorizationEndpoint =
    Uri.parse('https://github.com/login/oauth/authorize');
final _tockenEndpoint =
...
class _GithubLoginWidgetState extends State<GithubLoginWidget> {
        ...
        body: Center(
          child: ElevatedButton(
            onPressed: () async {
              ...
              var authenticatedHttpClient = await _getOAuth2Clent(
  ...
  Future<oauth2.Client> _getOAuth2Clent(Uri redirectUrl) async {
    if (widget.githubClientId.isEmpty || widget.githubClientSecret.isEmpty) {
      throw const GithubLoginException(
          'github client id and secret cannot be empty');
    }

    var grant = oauth2.AuthorizationCodeGrant(
        widget.githubClientId, _autorizationEndpoint, _tockenEndpoint,
  ...
  Future<void> _redirect(Uri authorizationUrl) async {
    if (await canLaunchUrl(authorizationUrl)) {
      ...
      throw GithubLoginException('Couldnot launch $authorizationUrl');
    }
  }
